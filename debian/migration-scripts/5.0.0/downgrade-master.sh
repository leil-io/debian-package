...
start_metalogger_server() {
    if [ "$WAS_METALOGGER_ACTIVE" -eq 1 ]; then
        ...
        log_message "saunafs-metalogger not installed, skipping start."
    fi
}
...
if [ "$IS_CHUNKSERVER_INSTALLED" -eq 0 ]; then
    if version_ge "${CHUNKSERVER_CURRENT_VERSION}" "5.0.0"; then
        log_message "saunafs-chunkserver version ${CHUNKSERVER_CURRENT_VERSION} is 5.0.0 or later. This script is not intended for downgrading chunkserver. Exiting."
        exit 1
    fi
fi
